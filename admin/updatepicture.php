...
function changeProfilePicture($id, $file, $ext, $link){
    $permanentdestination = 'picture/' . md5(time()) . ".$ext";
    if(move_uploaded_file($file, $permanentdestination)){
        $sql = "UPDATE garage SET picture='$permanentdestination' WHERE car_id='$id'";
        $result = mysqli_query($link, $sql);
        if(!$result){
            $resultMessage = '<div class="alert alert-danger">Unable to update profile picture. Please try again later.okay?</div>';
            echo $resultMessage;
        }
 }else{
...
if($errors){
    ...
    changeProfilePicture($id, $tmp_name, $extension, $link);
} 
